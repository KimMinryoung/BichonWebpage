(function () {
    ...
    function announce(text) { el('strikeAnnouncement').textContent = text; }
    function save() {
    ...
    function crews() {
        el('strikeSelection').textContent = state.phase === 'planning' ? `${state.crews[selected].name} 조 선택 중 · 아래 장소 버튼으로 배치` : '오늘의 배치와 피로';
        ...
        el('strikeSites').replaceChildren(...Object.entries(g.jobs).map(([job, info]) => {
            const count = state.crews.filter(c => c.job === job).length;
            const b = button('', () => {
                assign(job); el('strikeSites').querySelector('[data-job="' + job + '"]').focus({ preventScroll: true });
            }, 'strike-site');
            b.dataset.job = job; b.disabled = moving || state.phase !== 'planning';
            b.setAttribute('aria-pressed', String(state.crews[selected].job === job));
            b.append(node('strong', info.icon + ' ' + info.title), node('span', count + '개 조'), node('small', info.desc)); return b;
        }));
        if (window.matchMedia('(max-width: 760px)').matches) el('strikeCrews').after(el('strikeSites'));
        else el('strikeSceneSummary').before(el('strikeSites'));
        el('strikeHint').textContent = state.phase === 'planning' && state.day === 1 && !coached ? '① 동료 선택 → ② 장소 선택 → ③ 하루 보내기. 배치는 확정 전까지 바꿀 수 있어요.' : state.crews.some(c => c.fatigue >= 75 && c.job !== 'rest') ? '지친 조가 현장에 남아 있습니다. 휴식처로 교대해 주세요.' : state.unity < 20 ? '참여가 위태롭습니다. 내일도 20% 미만이면 종료됩니다. 조직 활동과 생활 지원이 필요합니다.' : '4 · 8 · 12일 납품 마감. 그날을 위한 힘도 남겨 두세요.';
    }
    function dealText(d) { return `임금 +${d.wage}% · 주 ${40 - d.hours}시간 · 강도 ${d.intensity ? '+' + d.intensity + '%' : '유지'}`; }
    function render() {
        ...
        meters(); crews(); window.StrikeScene.render(el('strikeScene'), state, selected, moving, assign);
        el('strikeSceneLabel').textContent = done ? '우리가 남긴 현장' : moving ? '현장에서 보내는 하루…' : review ? '하루가 끝난 현장' : '배치를 준비하는 아침';
        el('strikeSceneSummary').textContent = `생산 ${state.production}%. 밀린 출하 ${Math.ceil(state.backlog / 40)}대분. ${state.crews.filter(c => c.fatigue >= 75).length}개 조가 지쳐 있습니다.`;
        el('strikeAdvance').hidden = state.phase !== 'planning' || moving;
        el('strikeSkip').hidden = !moving;
        el('strikeForecast').textContent = moving ? '생산과 생활 지원의 결과를 확인하는 중…' : state.phase === 'planning' ? `피켓 ${state.crews.filter(c => c.job === 'picket').length} · 조직 ${state.crews.filter(c => c.job === 'organize').length} · 연대 ${state.crews.filter(c => c.job === 'solidarity').length} · 휴식 ${state.crews.filter(c => c.job === 'rest').length} / 하루가 지나면 생활비가 나갑니다.` : '오늘의 결정이 내일의 조건이 됩니다.';
        el('strikeReview').hidden = !review || moving;
        el('strikeResult').hidden = !done || moving;
        el('strikeOffers').hidden = !bargaining;
        if (review) {
            el('strikeDayReport').replaceChildren(...state.history[state.history.length - 1].messages.map(t => node('li', t)));
            ...
            el('strikeOfferCards').replaceChildren(...state.offers.map((d, index) => {
                ...
                card.append(node('h3', d.title), node('p', dealText(d)), node('p', `현재: 임금 인상 없음 · 주 40시간 · 강도 유지`, 'games-note'), node('p', d.intensity ? '임금은 더 오르지만 같은 시간에 15% 더 강하게 일합니다.' : d.wage >= 8 && d.hours >= 2 ? '세 가지 요구를 모두 충족합니다.' : '작업 강도는 지키지만 아직 충족하지 못한 요구가 있습니다.'));
        ...
        el('strikeLog').replaceChildren(...state.history.slice().reverse().map(r => node('li', `${r.day}일 · ${r.event} — ${r.messages.join(' ')}`)));
    }
    function finishAnimation() {
        clearTimeout(timer); moving = false; render();
        announce(state.history[state.history.length - 1].messages.join(' '));
    ...
    window.matchMedia('(max-width: 760px)').addEventListener('change', () => { if (state) render(); });
    load();
})();
