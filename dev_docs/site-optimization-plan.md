...
> - 2026-09-02: **배치 2 완료·배포** (커밋 abc4387, 12b7d05, fac0ca0, 2694618). CSRF 토큰을 세션 쿠키가 있거나 익명 폼 경로(/auth/login·/auth/signup·/admin/login)일 때만 발급(404·/health마다 Redis 세션 저장되던 것 종료; 스캐너 100회에 sess 수 불변), /health 세션 프리. sanitize-html 2.17.7·http-proxy-middleware 3.0.7 등 in-range 패치. Dockerfile node:20.20.2 고정·npm ci·HEALTHCHECK, engines/.nvmrc. deploy에 npm test → /health → 16경로 스윕 → drift 검사 게이트(실패 시 exit 1). dev-preview --user. marked 15.0.12 고정 + SRI + preconnect. 검증: 48/48 동일, CSRF 왕복(토큰 401/없음 403), javascript: URL 제거, 헤드리스에서 marked·DOMPurify 로드, docker health healthy. **미확인 1건: 어드민 패스키 로그인·로그아웃 실기기 확인(사용자). 실패 시 `git revert abc4387` 후 deploy, 복구는 scripts/reset-passkeys.js.** 호스트 node_modules가 root 소유라 `npm test`는 옛 모듈로 돌고 `npm ci`는 호스트에서 불가 — `sudo chown -R grass:grass node_modules` 필요.
> - 2026-09-02: **배치 3 완료·배포** (커밋 7933da3, e09fedc, 4006702). createDocRefResolver 세대별 메모, 문헌·사건 목록 presenter 메모(onPage 때문에 요청당 얕은 복사), 정치국 명부 (데이터, 인물 스냅숏, lang) 메모(요청당 DB 조회 0), politburo-store 500ms stat 디바운스, docs-store 본문 캐시 디바운스 + LRU 40, linkify 정규식 꼬리 5→1(`buildAliasPattern`, 해시 10/10 동일), 차단 목록 Set, `/en/` 카드 조각 링크 지역화(1,066 href 전부 /en/) + 클라이언트 /en 접두 fetch, `new URL()` 가드. 직책/역할 필터 메모는 이득이 작아 보류. 다음: 배치 4a(server.js 분해).
> - 2026-09-02: **배치 4a 완료·배포** (커밋 05c33aa). server.js 794→188줄: config/env.js(validateEnv 부팅 검증·intFromEnv 공용), config/route-policy.js, middleware/language.js, config/session.js, middleware/chat-identity.js, routes/chat-history.js+config/chat-log-store.js, config/proxies.js, config/security.js, middleware/view-locals.js, middleware/cache-policy.js, routes/redirects.js, config/static-assets.js, auth.js requireWriterAdminSession, http.js setNoStore. 검증: 48페이지 + 특수 경로 39 + 쿠키 매트릭스 13 동일(diff-preview COOKIE/NO_EN 옵션 추가). 다음: 배치 4b(page-helpers·requireUser·commulingo/reports 라우트 분할·people-admin-store 분할·bare 500 → 에러 페이지).
> - 2026-09-02: **배치 4b 완료** (커밋 c01528d, 4bed687, 21c2f8c, 705e088; 배포는 4단계 검증 뒤 한 번). page-helpers(캐시 헤더 21·빵부스러기 6·데이터 캐시 사본·bare 500 8곳 → 에러 페이지), requireUser 통합; routes/commulingo.js 1,049→83줄(people·people-api·books·progress 라우터 + data/commulingo/people-view.js·book-page.js); routes/reports.js 612→332줄(config/private-report-store.js, services/research-series.js·research-render.js; public.js getResearchFiles 사본 제거); people-admin-store.js 1,066→546줄(people-offices-store·people-sections-store·people-admin-validation·people-admin-fields·admin-tx). 분할은 원본 import 목록에서 실제 참조 이름만 자동 선별 + 정적 식별자 검사(researchMarkdown 누락 1건·getPersonAdmin 교차 참조 1건을 잡아냄). 검증: 기본 48 + CommuLingo 26+6 + 보고서 17+3 + 어드민 API 읽기/검증 경로. 보류: 페이지네이터 3종 통일(동작 변화), 직책/역할 필터 메모, task-reports 서비스 추출. ※ 어드민 API 검증 중 빈 페이로드가 유효 쓰기로 통과해 직책 행·절 각 1건이 생성됐다가 35초 내 삭제됨(revisions 이력 4행).
> - 2026-09-02: **배치 5 완료·배포**. views/public/commulingo-people.ejs 인라인 4블록(585줄, no-cache 페이지에서 매번 내려받던 30KB) → public/js/commulingo-people.js(PAGE_SIZE는 셸의 data-page-size, en은 html lang), views/admin/commulingo-docs.ejs CRUD 블록 → public/js/admin-commulingo-docs.js; escapeHtml 8곳 같은 본문으로(chat-logs·admin-users의 textContent 변형, edit-post의 따옴표 미이스케이프); public/js/nonogram.js IIFE 복원(전역 누출 0 확인); commulingo.css의 죽은 `.commu-volume-*` 규칙 18개(-114줄, grid는 commulingo.js가 만드는 살아 있는 클래스라 유지). **정정 2건**: common.js는 chat.ejs·commulingo-doc.ejs가 footer를 안 쓰므로 중복 로드가 아니었고(defer도 보류 — 날짜 텍스트 깜빡임 위험 대비 이득 없음), commu-volume-grid는 죽은 클래스가 아니었다. 검증: 헤드리스로 그룹 열기 24카드+페이저·검색 84건 205마크·초기화·#p- 포커스·노노그램 225칸·책 페이지 8그리드; escapeHtml 단위 출력 동일; 어드민 문헌 편집 화면은 로그인 필요라 사용자 확인 대기.
> - 2026-09-02: **배치 6 완료·배포**. 일회성 스크립트 3종 → scripts/one-off/(README; seed·wrap-fic-names·bake-event-basemap은 재실행 도구라 유지), scripts/lib/bootstrap.js(.env 로드 + db; drift 검사·일회성 DB 스크립트 적용), middleware/request-log.js(`LOG_REQUESTS=1` 한 줄 접근 로그), eslint.config.js(no-undef·no-unused-vars; 브라우저/노드/플레이라이트 전역) + scripts/lint(로컬 eslint 없으면 docker 폴백) + `npm run lint`(오류 0, 경고 11 잔존), scripts/test는 로컬 eslint가 있을 때만 lint 포함, CLAUDE.md 워크플로(npm test·diff-preview·deploy 게이트·bootstrap) 갱신, public.js·linkify·people-api의 죽은 import 정리. **사용자 sudo 작업 대기**: `sudo rm -rf data/post-cache data/report-cache data/diary-cache public/puzzles/.minchong-15.owner-nobody-backup` (Redis 이전 후 fs 리더 없음; cloudflare-analytics 일회성만 post-cache를 읽음), `sudo chown -R grass:grass node_modules config/hub-store.js views/admin/private-reports.ejs public/BingSiteAuth.xml data/commulingo/terms-snapshot.json scripts/audit-commulingo-quality.js scripts/cloudflare-purge.js scripts/build-commulingo-shards.js scripts/migrations docs` 뒤 `npm ci`로 호스트 node_modules 동기화(그러면 npm test에 lint 포함).
> - 2026-09-02: **배치 7 완료(코드·런타임 부분 배포, sudo 부분은 사용자)**. (1) Redis: `CONFIG SET maxmemory 512mb` + `volatile-lru` 런타임 적용, 영구 키 1,017개(research·report·post·diary 본문)에 30일 TTL 부여, 코드도 ENTRY_TTL 30일(config/report-cache.js·redis-entry-cache.js), leninbot 레포 docker-compose.neo4j.yml의 redis command에 같은 옵션 추가(**미커밋·미재기동** — 다음 `docker compose up -d redis` 때 영속). 전용 DB 인덱스 이전은 전원 로그아웃을 부르므로 보류(사용자 결정). (2) nginx/leninbot-frontend.conf: `listen 443 ssl http2`(1.24라 `http2 on` 불가), gzip, `upstream frontend_app { keepalive 16 }`, `/commulingo/api/`·`/api/proxy/`만 map 키로 10r/s burst 20 `limit_req`(429). docker nginx:1.24로 `nginx -t` 통과. **적용은 사용자**: `sudo cp nginx/leninbot-frontend.conf /etc/nginx/sites-available/leninbot-frontend && sudo nginx -t && sudo systemctl reload nginx` 후 `curl -sI --http2 https://cyber-lenin.com/css/style.css`로 확인. (3) people_revisions: maintainer의 LATERAL이 인물 2,175명마다 seq scan(폴링 1회 2,648ms; 121의 text_pattern_ops 인덱스는 비상수 LIKE 패턴에 못 씀). 표현식 부분 인덱스(마이그레이션 158, postgres 소유라 `psql -U postgres`로 적용 완료) + 술어 재작성 `split_part(rev.entity_id,'/',1) = p.id`로 14.5ms(결과 동일성 2,175명 전수 확인). **술어 재작성은 leninbot 레포 `scripts/commulingo_people_maintainer.py:347` — leninbot 세션에서.**
> - 2026-09-02: **배치 8 완료·배포**. 뷰 22개의 `en ? '…' : '…'` 단순 문자열 삼항 192곳을 `strings.commuLingoViews.<뷰>.<키>` 178개로(config/strings.js ko/en 양쪽; `commuLingo` 아래가 아니라 최상위인 이유는 `window.COMMULINGO_STRINGS`가 `strings.commuLingo` 전체를 직렬화하기 때문). 복합식 5곳(단·복수, 문자열 결합)은 그대로. 발견·수정: `req.app.render`로 그리는 조각 파셜(인물 카드·페이저)은 res.locals를 못 받아 `strings`를 명시적으로 넘겨야 했음(routes/commulingo-people.js). 검증: 기본 48 + 확장 30(목록·허브·계보·문헌·사건·용어·인물 상세·페이저) + 조각 5 전부 양 언어 바이트 동일; check-ui 전 뷰 컴파일. 뷰당 1커밋 대신 자동 변환 1커밋 + 전수 byte-diff.
...
> - **2차 패스 전체 완료(배치 0~9).** 남은 사용자 작업: nginx 적용, data/*-cache·puzzles 백업 sudo rm, root 소유 파일 chown + `npm ci`, leninbot compose 커밋·redis 재기동, maintainer 술어 재작성(leninbot), 어드민 패스키 로그인·문헌 편집 화면 실기기 확인.
>
