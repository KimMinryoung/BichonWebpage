...
`report-mentions.js`는 변경된 보고서의 역방향 연결만 제거/추가하고 기존 최신순 정렬을 유지한다. 사전 변경만으로는 보고서 원문을 다시 조회하지 않는다. 원문은 기존 10분 주기로 갱신하며, 보고서 수정·삭제·비공개 전환은 다음 조회에 반영된다. 재시작·캐시 퇴출 후에는 다시 계산한다. 빌드는 계속 백그라운드에서 수행하며, 현재 사전과 맞지 않는 이전 앵커는 완성 전까지 노출하지 않는다.
