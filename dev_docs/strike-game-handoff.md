...
작성일: 2026-09-06  
구현 기준: 시나리오 확장판 (저장 버전 4, 기존 v3 수식 유지; 운영 반영 여부는 컨테이너 revision으로 확인)
...
`start(mode, seed, scenario = 'standard')`는 시나리오를 검증한다. 아래 기본값은 standard 기준이다.

- standard: 기존 조건과 세 요구.
- tight: 기본/어려움 기금 50/40. 세 요구 합의와 함께 생활 지원 부족이 하루도 없어야 한다.
- rebuild: 참여율 40%, 초기 생산 60%. 세 요구 합의 시 참여율 70% 이상이어야 한다.
- 난이도별 생활비·교섭 보정과 12일 일정은 기존과 같다. `objectives()`가 체크리스트를 반환하고 `won()`은 추가 목표까지 검사한다.
- 시나리오 설명은 시작 화면에, 목표는 현장 위에 표시한다. 첫날 접이식 안내, 같은 시드/시나리오 재도전, 다른 시나리오 선택을 제공한다.


| 필드 | 의미/초기값 |
| --- | --- |
| `version`, `scenario`, `mode`, `seed` | 4, standard/tight/rebuild, normal/hard, unsigned 32-bit 사건 시드 |
...
하루 기록은 `day, event, income, cost, production, backlog, fund, unity, jobs, missing, exhausted, messages`를 저장한다. 결과 상자는 절대값과 전날 대비 증감을 함께 보여주며 1일차는 시작 상태와 비교한다. 기금 상자의 생활 지원액은 실제 지급액이다. 기록 표는 최신 일차부터 생산·미납·기금·참여의 **증감**만 보여준다.

현재 `narrative()`는 `messages`의 인덱스 1·2를 숫자 문장으로 간주해 제외한다. 엔진 메시지 순서를 바꾸면 이 결합을 반드시 확인한다. 구조화된 서술 필드로 바꿀 여지는 있으나 아직 구현하지 않았다.

저장은 `localStorage['strike-game-v4-save']` 한 슬롯이다.

- 새 게임, 하루 진행, 다음 날, 합의 시 저장한다.
- 드래그·터치·키보드로 배치를 확정할 때마다 즉시 저장한다. 드래그 중간 좌표는 저장하지 않는다. 진행 중인 판을 새 게임으로 바꾸기 전 확인한다.
...
v2 판을 현재 규칙으로 재생하면 결과가 달라지므로 **자동 이관하지 않는다**. v2 키는 삭제하지 않고 유효한 v4/v3 저장이 없을 때 규칙 개편과 새 게임 필요를 안내한다.

v4가 없거나 손상되면 유효한 v3 저장을 standard로 재생 복원해 v4에 저장한다. 원본 v3 키는 남긴다. v3 기본 수식과 순서를 유지하므로 자원·배치·교섭안은 동일하다. 고정된 실제 v3 fixture로 호환성을 검증한다. v4는 시나리오 ID도 검증하며 저장된 자원이나 추가 목표 통과 여부를 신뢰하지 않는다.
...
node scripts/test-games-ui.js
npm test
```

시나리오 3개 × 난이도 2개 × 시드 32개 = 추가 192개 캠페인에서 support 전략의 목표 달성 가능성과 복원을 검증한다. 부족 이력이 있는 tight와 참여율 69%인 rebuild의 실패도 검사한다.
