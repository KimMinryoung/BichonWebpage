...
개별 주석·제목의 id와 내부 링크에는 원래 문헌 id를 접두사로 붙여 중복을 막는다.
원문과 개별 서지는 보존한다. 목록·자동 링크·사이트맵에는 모음집만 등록한다.
...
- **해제 길이는 본문에 비례한다.** 해제는 독자가 본문으로 들어가는 문턱이지 본문의
  대용이 아니다. 짧은 문헌(전문·포고·의사록 몇 쪽)은 한 문단, 긴 문헌집이라도
  두세 문단을 넘기지 않는다. 사건 경위·역사적 배경은 사건 카드가 맡는다 — 해제에
  옮겨 적지 말 것(소유자 지적, 2026-09-01: 8천 자 전문 네 편에 3천 자 해제가 달려
  본문 읽을 맛을 떨어뜨렸다).
...
2. `manifest.json`의 `docs` 배열에 항목을 추가한다. **배열 어디에 넣어도 된다.**
   ...
   - `anchors` — `{"art-5": {ko: [], en: []}}`. 문헌 **안의 특정 절**로 가는
     별칭. 키는 fragment 제목의 `id`(번역 파이프라인은 스펙 문서의
     `headingIds` 규칙으로 `제5조` → `art-5`처럼 붙인다)이고, 값은 `aliases`와
     같은 언어별 표현 목록이다. 산문의 「NATO 5조」가 `/commulingo/docs/<id>#art-5`로
     걸리며, 절마다 첫 등장 한 번씩 따로 링크된다(한 보고서가 4조와 5조를 함께
     불러도 둘 다 걸린다). 관련 문헌 패널에는 문서가 한 번만 실린다. `aliases`와
     같은 표현이 겹치면 문서 전체 링크가 우선한다. 다른 표현과 마찬가지로
     링크 검토 승인을 받아야 자동 링크된다.
   - `addedAt` — YYYY-MM-DD
