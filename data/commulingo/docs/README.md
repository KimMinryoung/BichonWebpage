...
저본이 러시아어가 아니면 스펙에 `"sourceLang"`을 준다(`ru` 기본, `zh`, 그리고 2026-08-31부터
라틴 문자 언어 `en`·`de`·`fr`·`it`). 라틴 문자 저본은 미번역 잔여 검사가 「소문자를 품은
4자 이상 로마자 낱말」만 잡으므로 NATO·ICBM 같은 약어와 첫 등장의 괄호 원문은 통과하고,
용어표는 인물 `familyName.en`·용어 `term.en`(대문자 시작 고유명사꼴)에서 뽑힌다. 이 값이
...
2. `manifest.json`의 `docs` 배열에 항목을 추가한다. **배열 어디에 넣어도 된다.**
   ...
   예전에는 배열 순서가 곧 표시 순서였고 새 문헌을 제자리에 끼워 넣는 것이
   규약이었다. 그 규약은 문서에 적혀 있었는데도 두 번 깨졌다. 배열 끝에
   덧붙여도 화면에는 그럴듯하게 나오고 아무것도 항의하지 않기 때문이다.
   지금은 `date` 없는 항목이 목록 끝으로 밀리면서 콘솔에 이름이 찍힌다:
   - `id` — URL 슬러그 (파일명과 일치 권장)
   - `date` — **원전이 쓰인 날짜.** `YYYY` / `YYYY-MM` / `YYYY-MM-DD`. 여러
     문서를 묶은 문헌은 그중 가장 이른 것, 연구서처럼 원전이 아닌 문헌은
     저술 연도를 적는다. 연월일을 모르면 아는 데까지만 적으면 되고, 같은 해에서
   ...
   - `addedAt` — YYYY-MM-DD
   - `noAutoLink` — 선택. 이 문헌 본문에서 자동 링크를 걸지 않을 문자열 배열
     (`["임시정부", "전원회의"]`). 사전의 의미가 다른 데서는 맞지만 **이 문헌의
     문맥에서만 틀리는** 낱말용이다 — 프랑스 문서의 임시정부는 러시아 임시정부가
     아니다. 데이터 전용이라 배포 없이 반영된다. 거의 모든 페이지에서 틀리는
     별칭은 여기 말고 `commulingo_link_blocklist`의 `term-*` kind 행으로 전역
     차단한다(마이그레이션 154). 본문에 `소<!-- -->개`처럼 주석을 끼워 넣던
     낡은 수법은 이 필드로 대체됐다.
