...
function paginateList(items, matched, query, baseUrl, pageSize = PAGE_SIZE) {
    const total = Math.max(1, Math.ceil(matched.length / pageSize));
    let current = Number.parseInt(query && query.page, 10);
    if (!Number.isFinite(current) || current < 1) current = 1;
    if (current > total) current = total;
    const onPage = new Set(matched.slice((current - 1) * pageSize, current * pageSize));
    items.forEach(item => { item.onPage = onPage.has(item); });
    return { current, total, matched: matched.length, baseUrl, pageSize };
}
