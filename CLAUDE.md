...
- **CommuLingo people are registered through the Admin store, never by hand-written INSERTs**: `scripts/commulingo-people-upsert <spec.json> [--dry-run]` runs the same validation as `/admin/api/commulingo/people` (role required, short fate labels, nation-not-birthplace origin labels, patronymic kept out of the native name). After any hand-run SQL that touches `commulingo_people*`, run the four person audits (`audit-person-card-fields.js`, `audit-person-native-names.js`, `audit-person-patronymics.js`, `audit-person-name-order.js`) inside the container.
